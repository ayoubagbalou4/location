(function() {
  ...
  function initMapSingle() {
    const container = document.querySelector('.js-map-single');
    if (!container) return;

    // 1. Création de la carte centrée sur Tanger
    const map = new google.maps.Map(container, {
      zoom: 12,
      center: { lat: 35.76727, lng: -5.79975 }
    });

    // 2. Positions des marqueurs
    const locations = [
      { lat: 35.76727, lng: -5.79975 }
    ];

    // 3. Création de marqueurs HTML personnalisés
